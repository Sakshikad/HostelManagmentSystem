...
if (isset($_GET['id'])) {
    $rid = $_GET['id']; 
    $_SESSION['refid'] = $rid;  
    $sql = "select * from roomchange where ref_id='$rid'";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
  ...
  $rid= $_SESSION['refid'];
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($row2['status'] == "Available") {
            $sql1 = "UPDATE registration SET rooms_booked='$roomno' WHERE id='$rid'";
            $result1 = mysqli_query($conn, $sql1);
            if ($result1) {
                $sql2 = "UPDATE mainbook SET ref_id='$rid', name='$yname',regid='$regid',city='$city',status='Occupied' WHERE room='$roomno'";
                $result2 = mysqli_query($conn, $sql2);
                if ($result2) {
                    $sql3 = "DELETE FROM roomchange WHERE ref_id='$rid'";
                    $result3 = mysqli_query($conn, $sql3);
                    // echo "$result3";
                    echo "<script type='text/javascript'>alert('Room Booked Successfully !!!');document.location='room_reg.php';</script>";
                    $sql="delete from notification where reg_no ='$regid'";
                    $result = mysqli_query($conn, $sql);
                }
            } else {
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <form id="main" method="post" action="Allocation.php" enctype="multipart/form-data">
                                                <div class="row">
                                                    <div class="col-md-6">
